...
# work Mac) never has to download them directly from Hugging Face.
#
...
export)
    ...
    echo "ship it via Nexus, a file share, AirDrop, or USB, then: make install-models FILE=<path>"
    ;;
install)
    FILE="${2:-}"
    if [ ! -f "$FILE" ]; then
    ...
    ;;
*)
    echo "usage: models.sh {export|install <file>}" >&2
    exit 1
    ;;
esac
